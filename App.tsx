...
const INITIAL_CONFIG: CommandConfig = {
  ...
  extraVars: {},
  limit: 'all',
  forks: undefined, // Inactive by default
  diff: false,
  remoteUser: '',
  become: false,
  startAtTask: '',
      ...
      <header className="border-b border-slate-800 bg-slate-900/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
             <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-1 rounded font-mono border border-slate-700">STABLE v2.9</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 pt-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-7 space-y-8">
          ...
          <section>
             <button 
              onClick={() => {
                const isShowing = !showAdvanced;
                setShowAdvanced(isShowing);
                if (!isShowing) {
                  updateConfig({ forks: undefined });
                }
              }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input 
                        type="number" 
                        value={config.forks || ''} 
                        onChange={(e) => updateConfig({ forks: e.target.value === '' ? undefined : parseInt(e.target.value) })} 
                        placeholder="Désactivé"
                        className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-2 text-sm text-slate-200 focus:ring-1 focus:ring-indigo-500 outline-none" 
                      />
                    </div>
                  </div>
